...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (StaminaFill) StaminaFill.fillAmount = (MaxStamina > 0f) ? (_stamina / MaxStamina) : 0f;
        ...
        if (justLanded && impactSpeed >= LandMinSpeed)
        {
    ...
    {
        bool movingOnGround = _isGrounded && horizSpeed > MinStepSpeed && _moveDir.sqrMagnitude > 0.01f;

        if (!movingOnGround) _stepTimer = 0f;
        else
        {
            float targetInterval = _isSprinting ? StepIntervalSprint : StepIntervalWalk;
            float refSpeed = _isSprinting ? Mathf.Max(0.01f, SprintSpeed) : Mathf.Max(0.01f, WalkSpeed);
            float speedScale = Mathf.Clamp(horizSpeed / refSpeed, 0.2f, 2.5f);
            float interval = targetInterval / Mathf.Max(0.1f, speedScale);
